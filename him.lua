...
-- + Webhook tối giản: chỉ báo tổng Gems mỗi 10 phút (persist qua hop)
-- ================================================================
...
-- ======== WEBHOOK 10P (PERSIST QUA HOP): Chỉ báo tổng Gems =========
do
    local INTERVAL = 600 -- 10 phút
    getgenv()._WF_LAST_SENT = getgenv()._WF_LAST_SENT or 0
    ...
    local function sendOnce()
        local url = currentUrl(); if url == "" then return end
        local r   = req();        if not r   then return end

        local gems = readGems()
        local name = (Players.LocalPlayer and Players.LocalPlayer.Name) or "Player"
    ...
    task.spawn(function()
        while task.wait(1) do
            local now = os.time()
            if now - (getgenv()._WF_LAST_SENT or 0) >= INTERVAL then
                getgenv()._WF_LAST_SENT = now
                sendOnce()
            end
        end
    end)
