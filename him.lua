--// ===== CONFIG =====
local Config = {
    RegionFilterEnabled = false,                   -- bật lọc region
    RegionList = { "singapore", "tokyo", "us-east" },
    RetryHttpDelay = 2,                            -- delay khi request fail
}
...
local function Hop(mode)
    warn("[Hop] Đang tìm server để hop...")
    for i = 1, 5 do
    ...
    warn("[Hop] Không tìm thấy server phù hợp, thử lại sau.")
end
...
--// ===== RAINBOW BORDER + DIAMOND COUNTER =====
...
local a = Instance.new("ScreenGui", g:GetService("CoreGui"))
a.Name = "gg"
...
--// ===== TELEPORT & FARM =====
local function L_V1(cf) -- dịch chuyển HRP
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        LocalPlayer.Character:SetPrimaryPartCFrame(cf)
    end
end

local function L_V2() -- teleporters
...
local chestSeen = {}
local function L_V3() -- tìm chest gần nhất
...
local function L_V4() -- farm diamond
...
spawn(function()
    while task.wait(0.5) do
        if g.PlaceId~=126509999114328 then pcall(L_V2) end
    end
end)
...
spawn(function()
    while task.wait(0.1) do
        if g.PlaceId==126509999114328 then
            ...
            -- Kiểm tra in-active server để hop
            local inactive = true
